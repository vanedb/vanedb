...
fn hnsw_cosine_search() {
    let idx = ApproxIndex::builder(3, Metric::Cosine)
        .capacity(100)
        .seed(42)
        .build()
        .unwrap();
...
fn hnsw_concurrent_search() {
    ...
    let idx = Arc::new(
        ApproxIndex::builder(8, Metric::L2)
            .capacity(200)
            .seed(42)
            .build()
            .unwrap(),
...
fn hnsw_add_batch_matches_serial_add() {
    ...
    let mk = || {
        ApproxIndex::builder(dim, Metric::L2)
            .capacity(n)
            .m(8)
            .ef_construction(100)
            .seed(7)
...
fn builder_rejects_unallocatable_capacity() {
    let result = vanedb::ApproxIndex::builder(1024, vanedb::Metric::L2)

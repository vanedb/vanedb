...
### Cross-compile from Linux or macOS (NDK r26+)
...
Pass `--out-dir` under `/data/local/tmp` (the default
`$CARGO_MANIFEST_DIR/target/compare-out` is a build-host path and is not
writable on device). Use fixed `--json-out` names so `adb pull` is unambiguous:
...
adb shell 'cd /data/local/tmp && VANEDB_COMPARE_HW=android-arm64-device \
  ...
  --out-dir /data/local/tmp/compare-out \
  --json-out /data/local/tmp/device-cosine.json'
adb shell 'cd /data/local/tmp && VANEDB_COMPARE_HW=android-arm64-device \
  VANEDB_COMPARE_DEDICATED=1 \
  ./compare run --fixture embeddings.vnef --metric l2 --rounds 4 \
  --out-dir /data/local/tmp/compare-out \
  --json-out /data/local/tmp/device-l2.json'
...
# Or render-only:
# python3 bench/compare/scripts/render_comparison_md.py bench/compare/runs/device-cosine.json
# python3 bench/compare/scripts/render_comparison_md.py bench/compare/runs/device-l2.json
```
...
There is no env override for the pin. Still pass `--out-dir` as above.
...
`VANEDB_COMPARE_HW=android-arm64-device VANEDB_COMPARE_DEDICATED=1` and an
explicit `--out-dir` under a writable Termux path.
...
Pass `--out-dir` / `--json-out` the same way as on-device when using a
cross-compiled binary pushed via `adb`.

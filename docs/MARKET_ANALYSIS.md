...
unreleased) over application-owned ID lists and predicates. It does not
ship metadata storage, quantization, mobile SDKs, browser persistence, or a
comparison against any competitor. Every one of those is something the
...
| **VaneDB** | Rust, Python, C, wasm | yes (tombstone + compact) | yes (0.2.0, unreleased): predicate, allow/deny ID lists | no | VNDB v1/v2 file; not in wasm | Strongest loader hardening and format spec in the group |

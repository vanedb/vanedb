...
- a squared norm that underflows `f32` to zero, which needs *every* component
  at or below roughly 2.6e-23 (`2^-75`) — the bound is inclusive
  because `(2^-75)^2` is exactly `2^-150`, the midpoint between zero
  and the smallest subnormal, and a tie rounds to the even
  significand, which is zero — one ordinary component keeps the norm

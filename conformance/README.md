...
would pass every test. Demonstrated concretely — encoding cosine as `2` in one
engine leaves `bench/tests/cross_engine_format.rs` green, because both engines
then agree on the wrong meaning, while the fixture fails at offset 24.
